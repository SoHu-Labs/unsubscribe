...
- `tests/conftest.py` — `RUN_E2E` / `RUN_LIVE_BRAVE` + `GOOGLEADS_BROWSER_DEBUGGER_ADDRESS` (no deeplink gate).
...
- 1. **Copy** `conftest.py` → `tests/conftest.py` from neighbor. Fix imports. Reuse **`GOOGLEADS_BROWSER_DEBUGGER_ADDRESS`** for live Brave (same as neighbor). Keep CI gating logic identical.
...
| `GOOGLEADS_BROWSER_DEBUGGER_ADDRESS` | `GOOGLEADS_BROWSER_DEBUGGER_ADDRESS` (**same**; unsubscribe reads it too) | Brave remote debugging address (e.g. `127.0.0.1:9222`)     |
...
1. **Locate the session:** open the path printed as *Recording pages for format learning to …* (or search **`.unsubscribe_page_capture/session_<UTC>_<hash>/`** — base dir is from `page_capture_base_dir()` in `unsubscribe_page_capture.py`: repo root in a source tree, else your process cwd when the batch started).
